...
import {
  previewImportacionComparendos,
  ejecutarImportacionComparendos,
  type FilaPreviewComparendo,
  type ImportComparendosResult,
} from "@/lib/actions/importar-comparendos";
...
type Step =
  ...
  | { type: "done"; resultado: Extract<ImportComparendosResult, { ok: true }> };

export function ImportarComparendosForm() {
  ...
  const handlePreview = () => {
    ...
    startPreviewing(async () => {
      ...
      const result = await previewImportacionComparendos(fd);
      if (result.ok) {
        setStep({
          type: "preview",
          filas: result.filas,
  ...
  const handleConfirmImport = () => {
    ...
    startImporting(async () => {
      const fd = new FormData();
      for (const f of archivos) fd.append("archivos", f);
      const result = await ejecutarImportacionComparendos(fd);
      if (result.ok) {
  ...
  const handleDescargarSinMatch = () => {
    ...
    a.href = url;
    a.download = `comparendos-sin-match-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
  ...
  return (
    <div className="space-y-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              ...
              (ej: <code className="text-xs bg-muted rounded px-1">12345678.pdf</code>).
              El sistema buscará el proceso correspondiente y subirá el documento.
            </CardDescription>
          </CardHeader>
      ...
      {step.type === "preview" && (() => {
        ...
        const filasYaTieneDoc = step.filas.filter((f) => f.estado === "ya_tiene_doc");
          ...
          <>
            ...
            <Card className="border-green-200 dark:border-green-900">
              <CardHeader>
                ...
                <CardDescription>
                  Archivos con proceso coincidente sin documento previo de comparendo.
            ...
            <Card className="border-yellow-200 dark:border-yellow-900">
              <CardHeader>
                <CardTitle className="text-base text-yellow-700 dark:text-yellow-400">
                  Ya tienen documento — se omitirán (
                  {filasYaTieneDoc.length.toLocaleString("es-CO")})
                </CardTitle>
                <CardDescription>
                  El proceso ya tiene al menos un documento de comparendo registrado. Para
                  reemplazarlo, vaya al proceso y elimínelo manualmente.
                  {filasYaTieneDoc.length > limit &&
                    ` Mostrando los primeros ${limit.toLocaleString("es-CO")}.`}
                </CardDescription>
              </CardHeader>
              <CardContent>
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        <TableHead>N° Comparendo</TableHead>
                        <TableHead>Proceso / Contribuyente</TableHead>
                        <TableHead>Docs existentes</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {filasYaTieneDoc.length === 0 ? (
                        <TableRow>
                          <TableCell colSpan={4} className="text-center text-muted-foreground py-6">
                            Ningún proceso tiene documento previo.
                          </TableCell>
                        </TableRow>
                      ) : (
                        filasYaTieneDoc.slice(0, limit).map((fila, i) => (
                          <TableRow
                            key={`tiene-${i}`}
        ...
        <div className="space-y-4">
          <Card className="border-l-4 border-l-green-500">
            <CardHeader>
              ...
              <CardDescription>
                Los documentos PDF fueron subidos y registrados en los procesos correspondientes.
              </CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle className="flex items-center gap-2">
              ...
              ¿Confirmar importación de comparendos?
            </AlertDialogTitle>
            <AlertDialogDescription asChild>
              <div className="space-y-2 text-sm">
                <p>
                  Se subirán los PDFs que coinciden con un proceso y aún no tienen documento
                  de comparendo registrado. Los archivos sin match o con documento previo
                  serán omitidos.
                </p>
